...
class cls_mysql {
	...
	function __construct($dbhostu, $dbhosts, $dbuseru, $dbusers, $dbpwu, $dbpws, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 1) {
		$this->cls_mysql($dbhostu, $dbhosts, $dbuseru, $dbusers, $dbpwu, $dbpws, $dbname, $charset, $pconnect, $quiet);
	}

	function cls_mysql($dbhostu, $dbhosts, $dbuseru, $dbusers, $dbpwu, $dbpws, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 1) {
		...
		if ($quiet) {
			...
			$this->connect($dbhosts, $dbusers, $dbpws, $dbname, $charset, $pconnect, $quiet);
		} else {
			$this->settings = array (
				'dbhostu' => $dbhostu,
				'dbhosts' => $dbhosts,
	...
	function connect($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0) {
		if ($pconnect) {
			...
			if (PHP_VERSION >= '4.2') {
				...
				//die("<br>mysql_connect:031---(init)时间006：".$db_host_update.$db_user_select);
			} else {
				$this->link_id = @ mysql_connect($dbhost, $dbuser, $dbpw);

				mt_srand((double) microtime() * 1000000); // 对 PHP 4.2 以下的版本进行随机数函数的初始化工作
			} //die("<br>mysql_connect:032---(init)时间006：".$db_host_update.$db_user_select);
			if (!$this->link_id) {
				if (!$quiet) {
		...
		//die("<br>mysql_connect:002---(init)时间006：".$db_host_update.$db_user_select);
	...
	function master_connect($dbhost, $dbuser, $dbpw, $dbname = '', $charset = 'utf8', $pconnect = 0, $quiet = 0) {
		if ($pconnect) {
			if (!($this->master_link_id = @ mysql_pconnect($dbhost, $dbuser, $dbpw))) {
				if (!$quiet) {
					$this->ErrorMsg("Can't pConnect MySQL Server($dbhost)!");
				}
			...
			if (PHP_VERSION >= '4.2') {
				$this->master_link_id = @ mysql_connect($dbhost, $dbuser, $dbpw, true);
			} else {
				$this->master_link_id = @ mysql_connect($dbhost, $dbuser, $dbpw);

				mt_srand((double) microtime() * 1000000); // 对 PHP 4.2 以下的版本进行随机数函数的初始化工作
			}
			if (!$this->master_link_id) {
				if (!$quiet) {
					$this->ErrorMsg("Can't Connect MySQL Server($dbhost)!");
				}
		...
		$this->dbhash = md5($this->root_path . $dbhost . $dbuser . $dbpw . $dbname);
		$this->version = mysql_get_server_info($this->master_link_id);

		/* 如果mysql 版本是 4.1+ 以上，需要对字符集进行初始化 */
		if ($this->version > '4.1') {
			if ($charset != 'latin1') {
				mysql_query("SET character_set_connection=$charset, character_set_results=$charset, character_set_client=binary", $this->master_link_id);
			}
			if ($this->version > '5.0.1') {
				mysql_query("SET sql_mode=''", $this->master_link_id);
			}
		}
		...
		if ($this->max_cache_time && $this->starttime > $this->mysql_config_cache_file_time + $this->max_cache_time) {
			if ($dbhost != '.') {
				$result = mysql_query("SHOW VARIABLES LIKE 'basedir'", $this->master_link_id);
				$row = mysql_fetch_assoc($result);
				if (!empty ($row['Value'] { 1 }) && $row['Value'] { 1 }
					== ':' && !empty ($row['Value'] { 2 }) && $row['Value'] { 2 }
					== "\\") {
			...
			if ($this->platform == 'OTHER' && ($dbhost != '.' && strtolower($dbhost) != 'localhost:3306' && $dbhost != '127.0.0.1:3306') || (PHP_VERSION >= '5.1' && date_default_timezone_get() == 'UTC')) {
				$result = mysql_query("SELECT UNIX_TIMESTAMP() AS timeline, UNIX_TIMESTAMP('" . date('Y-m-d H:i:s', $this->starttime) . "') AS timezone", $this->master_link_id);
				$row = mysql_fetch_assoc($result);
		...
		/* 选择数据库 */
		if ($dbname) {
			if (mysql_select_db($dbname, $this->master_link_id) === false) {
				if (!$quiet) {
					$this->ErrorMsg("Can't select MySQL database($dbname)!");
				}
	...
	function select_database($dbname, $link_id = 0) {
		if ($link_id)
			return mysql_select_db($dbname, $link_id);
		return mysql_select_db($dbname, $this->link_id);
	}

	function set_mysql_charset($charset, $link_id = 0) {
		/* 如果mysql 版本是 4.1+ 以上，需要对字符集进行初始化 */
		if ($this->version > '4.1') {
			...
			if ($charset != 'latin1') {
				if ($link_id)
					mysql_query("SET character_set_connection=$charset, character_set_results=$charset, character_set_client=binary", $link_id);
				else
					mysql_query("SET character_set_connection=$charset, character_set_results=$charset, character_set_client=binary", $this->link_id);
			}
		}
	...
	*************************************/
	...
	function query_update($sql, $type = '', $quiet = 1) {
		if ($this->master_link_id === NULL) {
			$this->master_connect($this->settings['dbhostu'], $this->settings['dbuseru'], $this->settings['dbpwu'], $this->settings['dbname'], $this->settings['charset'], $this->settings['pconnect']);
			$this->settings = array ();
		}
		...
		if (PHP_VERSION >= '4.3' && time() > $this->starttime + 1) {
			mysql_ping($this->master_link_id);
		}

		if (!($query = mysql_query($sql, $this->master_link_id)) && $type != 'SILENT') {
			$this->error_message[]['message'] = 'MySQL Query Error';
			$this->error_message[]['sql'] = $sql;
			$this->error_message[]['error'] = mysql_error($this->master_link_id);
			$this->error_message[]['errno'] = mysql_errno($this->master_link_id);
			if (!$quiet) {
				$this->ErrorMsg();
			}
			return false;
	...
	function query($sql, $type = '') {
		return $this->query_update($sql, $type);
	}

	function query_select($sql, $type = '', $quiet = 1) {
		if ($this->master_link_id === NULL) {
			$this->master_connect($this->settings['dbhostu'], $this->settings['dbuseru'], $this->settings['dbpwu'], $this->settings['dbname'], $this->settings['charset'], $this->settings['pconnect']);
			$this->settings = array ();
		}
	...
	function affected_rows($link_id = 0) {
		if ($link_id)
			return mysql_affected_rows($link_id);
		return mysql_affected_rows($this->link_id);
	}

	function error($link_id = 0) {
		if ($link_id)
	...
	function insert_id($link_id = 0) {
		...
		return mysql_insert_id($this->master_link_id);
	}
	...
	function ping($link_id = 0) {
		if (PHP_VERSION >= '4.3') {
			if ($link_id)
				return mysql_ping($link_id);
	...
	function ErrorMsg($message = '', $sql = '') {
		...
		if ($message) {
			echo "<b>VRM info</b>: $message-用户名：" . $this->settings['dbuseru'] . " 密码：" . $this->settings['dbpwu'] . "\n\n";
		} else {
	...
	function table_lastupdate($tables) {
		if ($this->link_id === NULL) {
			$this->connect($this->settings['dbhostu'], $this->settings['dbuseru'], $this->settings['dbpwu'], $this->settings['dbname'], $this->settings['charset'], $this->settings['pconnect']);
			$this->settings = array ();
		}
