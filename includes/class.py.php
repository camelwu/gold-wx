...
function wordscut($string,$length) {
	...
	if(ord($string[$i]) > 127) {
	$wordscut= $wordscut.$string[$i].$string[$i + 1];
	$i++;
	} else {
	$wordscut= $wordscut.$string[$i];
	}
	}
	return $wordscut."...";
	}
...
function htmlencode2($msgstr){//转化字符串
	if(empty($msgstr)){
	return "";
	}else{
	...
	//$msgstr = str_replace("<","&lt;",$msgstr);
	//$msgstr = str_replace(">","&gt;",$msgstr);
	$msgstr = str_replace(chr(13),"<br>",$msgstr);
	$msgstr = str_replace("    ",chr(9),$msgstr);
	$msgstr = str_replace(chr(9),"　",$msgstr);
	return $msgstr;
	}
}
function killpath($strchar){
	if(empty($strchar)){
	return "";
	}else{
	$strchar = trim($strchar);
		$string = explode('[/]',$strchar);
		$strchar = end($string);
		$string = explode('[\]',$strchar);
		$strchar = end($string);
		return $strchar;
